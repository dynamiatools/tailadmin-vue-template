...
import Component from '@dynamia-tools/tailadmin-vue/components/forms/FormElements/RadioInputs.vue'
...
export const RadioInputsElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-radio-inputs')) {
  customElements.define('ta-radio-inputs', RadioInputsElement)
}
...
//
// Not derived from `InstanceType<typeof RadioInputsElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `RadioInputsElement`'s type, not a lazy type alias), that conditional type
...
export type RadioInputsProps = ComponentProps<typeof Component>
