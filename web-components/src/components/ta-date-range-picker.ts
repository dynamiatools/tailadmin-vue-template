...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/scheduling/DateRangePicker.vue'
...
export const DateRangePickerElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-date-range-picker')) {
  customElements.define('ta-date-range-picker', DateRangePickerElement)
}
...
//
// Not derived from `InstanceType<typeof DateRangePickerElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `DateRangePickerElement`'s type, not a lazy type alias), that conditional type
...
export type DateRangePickerProps = ComponentProps<typeof Component>
