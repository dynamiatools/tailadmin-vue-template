...
import Component from '@dynamia-tools/tailadmin-vue/components/ecommerce/CustomerDemographic.vue'
...
export const CustomerDemographicElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-customer-demographic')) {
  customElements.define('ta-customer-demographic', CustomerDemographicElement)
}
...
//
// Not derived from `InstanceType<typeof CustomerDemographicElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `CustomerDemographicElement`'s type, not a lazy type alias), that conditional type
...
export type CustomerDemographicProps = ComponentProps<typeof Component>
