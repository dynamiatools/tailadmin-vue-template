...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/display/Timeline.vue'
...
export const TimelineElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-timeline')) {
  customElements.define('ta-timeline', TimelineElement)
}
...
//
// Not derived from `InstanceType<typeof TimelineElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `TimelineElement`'s type, not a lazy type alias), that conditional type
...
export type TimelineProps = ComponentProps<typeof Component>
