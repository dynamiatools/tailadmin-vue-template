...
import Component from '@dynamia-tools/tailadmin-vue/components/ui/YouTubeEmbed.vue'
...
export const YoutubeEmbedElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-youtube-embed')) {
  customElements.define('ta-youtube-embed', YoutubeEmbedElement)
}
...
//
// Not derived from `InstanceType<typeof YoutubeEmbedElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `YoutubeEmbedElement`'s type, not a lazy type alias), that conditional type
...
export type YoutubeEmbedProps = ComponentProps<typeof Component>
