...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/EmptyState.vue'
...
export const EmptyStateElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-empty-state')) {
  customElements.define('ta-empty-state', EmptyStateElement)
}
...
//
// Not derived from `InstanceType<typeof EmptyStateElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `EmptyStateElement`'s type, not a lazy type alias), that conditional type
...
export type EmptyStateProps = ComponentProps<typeof Component>
