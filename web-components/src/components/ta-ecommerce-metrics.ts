...
import Component from '@dynamia-tools/tailadmin-vue/components/ecommerce/EcommerceMetrics.vue'
...
export const EcommerceMetricsElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-ecommerce-metrics')) {
  customElements.define('ta-ecommerce-metrics', EcommerceMetricsElement)
}
...
//
// Not derived from `InstanceType<typeof EcommerceMetricsElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `EcommerceMetricsElement`'s type, not a lazy type alias), that conditional type
...
export type EcommerceMetricsProps = ComponentProps<typeof Component>
