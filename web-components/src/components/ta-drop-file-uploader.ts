...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/media/DropFileUploader.vue'
...
export const DropFileUploaderElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-drop-file-uploader')) {
  customElements.define('ta-drop-file-uploader', DropFileUploaderElement)
}
...
//
// Not derived from `InstanceType<typeof DropFileUploaderElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `DropFileUploaderElement`'s type, not a lazy type alias), that conditional type
...
export type DropFileUploaderProps = ComponentProps<typeof Component>
