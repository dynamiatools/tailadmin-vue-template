...
import Component from '@dynamia-tools/tailadmin-vue/components/tables/TableHeader.vue'
...
export const TableHeaderElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-table-header')) {
  customElements.define('ta-table-header', TableHeaderElement)
}
...
//
// Not derived from `InstanceType<typeof TableHeaderElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `TableHeaderElement`'s type, not a lazy type alias), that conditional type
...
export type TableHeaderProps = ComponentProps<typeof Component>
