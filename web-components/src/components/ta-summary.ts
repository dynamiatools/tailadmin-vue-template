...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/display/Summary.vue'
...
export const SummaryElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-summary')) {
  customElements.define('ta-summary', SummaryElement)
}
...
//
// Not derived from `InstanceType<typeof SummaryElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `SummaryElement`'s type, not a lazy type alias), that conditional type
...
export type SummaryProps = ComponentProps<typeof Component>
