...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/PricingTable.vue'
...
export const PricingTableElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-pricing-table')) {
  customElements.define('ta-pricing-table', PricingTableElement)
}
...
//
// Not derived from `InstanceType<typeof PricingTableElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `PricingTableElement`'s type, not a lazy type alias), that conditional type
...
export type PricingTableProps = ComponentProps<typeof Component>
