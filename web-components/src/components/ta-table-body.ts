...
import Component from '@dynamia-tools/tailadmin-vue/components/tables/TableBody.vue'
...
export const TableBodyElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-table-body')) {
  customElements.define('ta-table-body', TableBodyElement)
}
...
//
// Not derived from `InstanceType<typeof TableBodyElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `TableBodyElement`'s type, not a lazy type alias), that conditional type
...
export type TableBodyProps = ComponentProps<typeof Component>
