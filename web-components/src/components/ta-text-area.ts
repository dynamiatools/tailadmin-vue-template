...
import Component from '@dynamia-tools/tailadmin-vue/components/forms/FormElements/TextArea.vue'
...
export const TextAreaElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-text-area')) {
  customElements.define('ta-text-area', TextAreaElement)
}
...
//
// Not derived from `InstanceType<typeof TextAreaElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `TextAreaElement`'s type, not a lazy type alias), that conditional type
