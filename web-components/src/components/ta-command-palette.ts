...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/navigation/CommandPalette.vue'
...
export const CommandPaletteElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-command-palette')) {
  customElements.define('ta-command-palette', CommandPaletteElement)
}
...
//
// Not derived from `InstanceType<typeof CommandPaletteElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `CommandPaletteElement`'s type, not a lazy type alias), that conditional type
...
export type CommandPaletteProps = ComponentProps<typeof Component>
