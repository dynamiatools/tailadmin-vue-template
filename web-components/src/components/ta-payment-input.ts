...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/input/PaymentInput.vue'
...
export const PaymentInputElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-payment-input')) {
  customElements.define('ta-payment-input', PaymentInputElement)
}
...
//
// Not derived from `InstanceType<typeof PaymentInputElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `PaymentInputElement`'s type, not a lazy type alias), that conditional type
...
export type PaymentInputProps = ComponentProps<typeof Component>
