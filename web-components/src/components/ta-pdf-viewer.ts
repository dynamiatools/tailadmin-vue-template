...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/display/PdfViewer.vue'
...
export const PdfViewerElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-pdf-viewer')) {
  customElements.define('ta-pdf-viewer', PdfViewerElement)
}
...
//
// Not derived from `InstanceType<typeof PdfViewerElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `PdfViewerElement`'s type, not a lazy type alias), that conditional type
...
export type PdfViewerProps = ComponentProps<typeof Component>
