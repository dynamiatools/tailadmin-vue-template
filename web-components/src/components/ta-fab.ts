...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/navigation/Fab.vue'
...
export const FabElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-fab')) {
  customElements.define('ta-fab', FabElement)
}
...
//
// Not derived from `InstanceType<typeof FabElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `FabElement`'s type, not a lazy type alias), that conditional type
...
export type FabProps = ComponentProps<typeof Component>
