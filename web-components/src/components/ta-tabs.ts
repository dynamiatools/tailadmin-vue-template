...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/Tabs.vue'
...
export const TabsElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-tabs')) {
  customElements.define('ta-tabs', TabsElement)
}
...
//
// Not derived from `InstanceType<typeof TabsElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `TabsElement`'s type, not a lazy type alias), that conditional type
...
export type TabsProps = ComponentProps<typeof Component>
