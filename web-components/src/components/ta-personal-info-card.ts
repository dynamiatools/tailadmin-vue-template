...
import Component from '@dynamia-tools/tailadmin-vue/components/profile/PersonalInfoCard.vue'
...
export const PersonalInfoCardElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-personal-info-card')) {
  customElements.define('ta-personal-info-card', PersonalInfoCardElement)
}
...
//
// Not derived from `InstanceType<typeof PersonalInfoCardElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `PersonalInfoCardElement`'s type, not a lazy type alias), that conditional type
...
export type PersonalInfoCardProps = ComponentProps<typeof Component>
