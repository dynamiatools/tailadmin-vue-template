...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/FAQAccordion.vue'
...
export const FaqAccordionElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-faq-accordion')) {
  customElements.define('ta-faq-accordion', FaqAccordionElement)
}
...
//
// Not derived from `InstanceType<typeof FaqAccordionElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `FaqAccordionElement`'s type, not a lazy type alias), that conditional type
...
export type FaqAccordionProps = ComponentProps<typeof Component>
