...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/BorderLayout.vue'
...
export const BorderLayoutElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-border-layout')) {
  customElements.define('ta-border-layout', BorderLayoutElement)
}
...
//
// Not derived from `InstanceType<typeof BorderLayoutElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `BorderLayoutElement`'s type, not a lazy type alias), that conditional type
...
export type BorderLayoutProps = ComponentProps<typeof Component>
