...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/scheduling/Calendar.vue'
...
export const CalendarElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-calendar')) {
  customElements.define('ta-calendar', CalendarElement)
}
...
//
// Not derived from `InstanceType<typeof CalendarElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `CalendarElement`'s type, not a lazy type alias), that conditional type
...
export type CalendarProps = ComponentProps<typeof Component>
