...
import Component from '@dynamia-tools/tailadmin-vue/components/forms/FormElements/InputState.vue'
...
export const InputStateElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-input-state')) {
  customElements.define('ta-input-state', InputStateElement)
}
...
//
// Not derived from `InstanceType<typeof InputStateElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `InputStateElement`'s type, not a lazy type alias), that conditional type
...
export type InputStateProps = ComponentProps<typeof Component>
