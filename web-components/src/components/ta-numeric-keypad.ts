...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/input/NumericKeypad.vue'
...
export const NumericKeypadElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-numeric-keypad')) {
  customElements.define('ta-numeric-keypad', NumericKeypadElement)
}
...
//
// Not derived from `InstanceType<typeof NumericKeypadElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `NumericKeypadElement`'s type, not a lazy type alias), that conditional type
...
export type NumericKeypadProps = ComponentProps<typeof Component>
