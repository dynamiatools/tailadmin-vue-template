...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/FeatureGrid.vue'
...
export const FeatureGridElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-feature-grid')) {
  customElements.define('ta-feature-grid', FeatureGridElement)
}
...
//
// Not derived from `InstanceType<typeof FeatureGridElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `FeatureGridElement`'s type, not a lazy type alias), that conditional type
...
export type FeatureGridProps = ComponentProps<typeof Component>
