...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/Navbar.vue'
...
export const NavbarElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-navbar')) {
  customElements.define('ta-navbar', NavbarElement)
}
...
//
// Not derived from `InstanceType<typeof NavbarElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `NavbarElement`'s type, not a lazy type alias), that conditional type
...
export type NavbarProps = ComponentProps<typeof Component>
