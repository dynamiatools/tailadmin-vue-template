...
import Component from '@dynamia-tools/tailadmin-vue/components/forms/FormElements/SelectInput.vue'
...
export const SelectInputElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-select-input')) {
  customElements.define('ta-select-input', SelectInputElement)
}
...
//
// Not derived from `InstanceType<typeof SelectInputElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `SelectInputElement`'s type, not a lazy type alias), that conditional type
...
export type SelectInputProps = ComponentProps<typeof Component>
