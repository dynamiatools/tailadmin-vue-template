...
import Component from '@dynamia-tools/tailadmin-vue/components/common/ComponentCard.vue'
...
export const ComponentCardElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-component-card')) {
  customElements.define('ta-component-card', ComponentCardElement)
}
...
//
// Not derived from `InstanceType<typeof ComponentCardElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ComponentCardElement`'s type, not a lazy type alias), that conditional type
...
export type ComponentCardProps = ComponentProps<typeof Component>
