...
import Component from '@dynamia-tools/tailadmin-vue/components/ecommerce/MonthlySale.vue'
...
export const MonthlySaleElement = defineCustomElement(Component, {
  shadowRoot: false,
  configureApp: registerRouterLinkFallback,
})

if (typeof customElements !== 'undefined' && !customElements.get('ta-monthly-sale')) {
  customElements.define('ta-monthly-sale', MonthlySaleElement)
}
...
//
// Not derived from `InstanceType<typeof MonthlySaleElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `MonthlySaleElement`'s type, not a lazy type alias), that conditional type
...
export type MonthlySaleProps = ComponentProps<typeof Component>
