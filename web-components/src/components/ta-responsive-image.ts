...
import Component from '@dynamia-tools/tailadmin-vue/components/ui/images/ResponsiveImage.vue'
...
export const ResponsiveImageElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-responsive-image')) {
  customElements.define('ta-responsive-image', ResponsiveImageElement)
}
...
//
// Not derived from `InstanceType<typeof ResponsiveImageElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ResponsiveImageElement`'s type, not a lazy type alias), that conditional type
...
export type ResponsiveImageProps = ComponentProps<typeof Component>
