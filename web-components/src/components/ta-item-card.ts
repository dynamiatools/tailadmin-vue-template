...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/commerce/ItemCard.vue'
...
export const ItemCardElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-item-card')) {
  customElements.define('ta-item-card', ItemCardElement)
}
...
//
// Not derived from `InstanceType<typeof ItemCardElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ItemCardElement`'s type, not a lazy type alias), that conditional type
...
export type ItemCardProps = ComponentProps<typeof Component>
