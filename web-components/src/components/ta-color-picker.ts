...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/input/ColorPicker.vue'
...
export const ColorPickerElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-color-picker')) {
  customElements.define('ta-color-picker', ColorPickerElement)
}
...
//
// Not derived from `InstanceType<typeof ColorPickerElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ColorPickerElement`'s type, not a lazy type alias), that conditional type
...
export type ColorPickerProps = ComponentProps<typeof Component>
