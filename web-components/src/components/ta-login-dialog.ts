...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/LoginDialog.vue'
...
export const LoginDialogElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-login-dialog')) {
  customElements.define('ta-login-dialog', LoginDialogElement)
}
...
//
// Not derived from `InstanceType<typeof LoginDialogElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `LoginDialogElement`'s type, not a lazy type alias), that conditional type
...
export type LoginDialogProps = ComponentProps<typeof Component>
