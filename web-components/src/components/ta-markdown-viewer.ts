...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/MarkdownViewer.vue'
...
export const MarkdownViewerElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-markdown-viewer')) {
  customElements.define('ta-markdown-viewer', MarkdownViewerElement)
}
...
//
// Not derived from `InstanceType<typeof MarkdownViewerElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `MarkdownViewerElement`'s type, not a lazy type alias), that conditional type
...
export type MarkdownViewerProps = ComponentProps<typeof Component>
