...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/CTA.vue'
...
export const CtaElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-cta')) {
  customElements.define('ta-cta', CtaElement)
}
...
//
// Not derived from `InstanceType<typeof CtaElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `CtaElement`'s type, not a lazy type alias), that conditional type
...
export type CtaProps = ComponentProps<typeof Component>
