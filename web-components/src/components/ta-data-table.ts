...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/data/DataTable.vue'
...
export const DataTableElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-data-table')) {
  customElements.define('ta-data-table', DataTableElement)
}
...
//
// Not derived from `InstanceType<typeof DataTableElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `DataTableElement`'s type, not a lazy type alias), that conditional type
...
export type DataTableProps = ComponentProps<typeof Component>
