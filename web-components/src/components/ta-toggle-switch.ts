...
import Component from '@dynamia-tools/tailadmin-vue/components/forms/FormElements/ToggleSwitch.vue'
...
export const ToggleSwitchElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-toggle-switch')) {
  customElements.define('ta-toggle-switch', ToggleSwitchElement)
}
...
//
// Not derived from `InstanceType<typeof ToggleSwitchElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ToggleSwitchElement`'s type, not a lazy type alias), that conditional type
...
export type ToggleSwitchProps = ComponentProps<typeof Component>
