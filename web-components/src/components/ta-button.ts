...
import Component from '@dynamia-tools/tailadmin-vue/components/ui/Button.vue'
...
export const ButtonElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-button')) {
  customElements.define('ta-button', ButtonElement)
}
...
//
// Not derived from `InstanceType<typeof ButtonElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ButtonElement`'s type, not a lazy type alias), that conditional type
...
export type ButtonProps = ComponentProps<typeof Component>
