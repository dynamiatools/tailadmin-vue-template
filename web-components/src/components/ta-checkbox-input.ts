...
import Component from '@dynamia-tools/tailadmin-vue/components/forms/FormElements/CheckboxInput.vue'
...
export const CheckboxInputElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-checkbox-input')) {
  customElements.define('ta-checkbox-input', CheckboxInputElement)
}
...
//
// Not derived from `InstanceType<typeof CheckboxInputElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `CheckboxInputElement`'s type, not a lazy type alias), that conditional type
...
export type CheckboxInputProps = ComponentProps<typeof Component>
