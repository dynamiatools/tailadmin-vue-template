...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/commerce/PrintPreview.vue'
...
export const PrintPreviewElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-print-preview')) {
  customElements.define('ta-print-preview', PrintPreviewElement)
}
...
//
// Not derived from `InstanceType<typeof PrintPreviewElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `PrintPreviewElement`'s type, not a lazy type alias), that conditional type
...
export type PrintPreviewProps = ComponentProps<typeof Component>
