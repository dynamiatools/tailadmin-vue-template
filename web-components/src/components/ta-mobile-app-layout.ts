...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/MobileAppLayout.vue'
...
export const MobileAppLayoutElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-mobile-app-layout')) {
  customElements.define('ta-mobile-app-layout', MobileAppLayoutElement)
}
...
//
// Not derived from `InstanceType<typeof MobileAppLayoutElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `MobileAppLayoutElement`'s type, not a lazy type alias), that conditional type
...
export type MobileAppLayoutProps = ComponentProps<typeof Component>
