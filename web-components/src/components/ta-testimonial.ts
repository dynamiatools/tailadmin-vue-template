...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/Testimonial.vue'
...
export const TestimonialElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-testimonial')) {
  customElements.define('ta-testimonial', TestimonialElement)
}
...
//
// Not derived from `InstanceType<typeof TestimonialElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `TestimonialElement`'s type, not a lazy type alias), that conditional type
...
export type TestimonialProps = ComponentProps<typeof Component>
