...
import Component from '@dynamia-tools/tailadmin-vue/components/forms/FormElements/DefaultInputs.vue'
...
export const DefaultInputsElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-default-inputs')) {
  customElements.define('ta-default-inputs', DefaultInputsElement)
}
...
//
// Not derived from `InstanceType<typeof DefaultInputsElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `DefaultInputsElement`'s type, not a lazy type alias), that conditional type
...
export type DefaultInputsProps = ComponentProps<typeof Component>
