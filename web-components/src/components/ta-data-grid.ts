...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/data/DataGrid.vue'
...
export const DataGridElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-data-grid')) {
  customElements.define('ta-data-grid', DataGridElement)
}
...
//
// Not derived from `InstanceType<typeof DataGridElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `DataGridElement`'s type, not a lazy type alias), that conditional type
...
export type DataGridProps = ComponentProps<typeof Component>
