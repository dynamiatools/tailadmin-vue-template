...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/data/EntityAutocomplete.vue'
...
export const EntityAutocompleteElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-entity-autocomplete')) {
  customElements.define('ta-entity-autocomplete', EntityAutocompleteElement)
}
...
//
// Not derived from `InstanceType<typeof EntityAutocompleteElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `EntityAutocompleteElement`'s type, not a lazy type alias), that conditional type
...
export type EntityAutocompleteProps = ComponentProps<typeof Component>
