...
import Component from '@dynamia-tools/tailadmin-vue/components/charts/LineChart/LineChartOne.vue'
...
export const LineChartOneElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-line-chart-one')) {
  customElements.define('ta-line-chart-one', LineChartOneElement)
}
...
//
// Not derived from `InstanceType<typeof LineChartOneElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `LineChartOneElement`'s type, not a lazy type alias), that conditional type
...
export type LineChartOneProps = ComponentProps<typeof Component>
