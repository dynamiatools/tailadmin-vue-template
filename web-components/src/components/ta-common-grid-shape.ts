...
import Component from '@dynamia-tools/tailadmin-vue/components/common/CommonGridShape.vue'
...
export const CommonGridShapeElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-common-grid-shape')) {
  customElements.define('ta-common-grid-shape', CommonGridShapeElement)
}
...
//
// Not derived from `InstanceType<typeof CommonGridShapeElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `CommonGridShapeElement`'s type, not a lazy type alias), that conditional type
...
export type CommonGridShapeProps = ComponentProps<typeof Component>
