...
import Component from '@dynamia-tools/tailadmin-vue/components/ecommerce/StatisticsChart.vue'
...
export const StatisticsChartElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-statistics-chart')) {
  customElements.define('ta-statistics-chart', StatisticsChartElement)
}
...
//
// Not derived from `InstanceType<typeof StatisticsChartElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `StatisticsChartElement`'s type, not a lazy type alias), that conditional type
...
export type StatisticsChartProps = ComponentProps<typeof Component>
