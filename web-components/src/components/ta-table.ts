...
import Component from '@dynamia-tools/tailadmin-vue/components/tables/Table.vue'
...
export const TableElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-table')) {
  customElements.define('ta-table', TableElement)
}
...
//
// Not derived from `InstanceType<typeof TableElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `TableElement`'s type, not a lazy type alias), that conditional type
...
export type TableProps = ComponentProps<typeof Component>
