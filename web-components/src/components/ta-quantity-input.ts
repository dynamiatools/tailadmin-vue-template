...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/input/QuantityInput.vue'
...
export const QuantityInputElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-quantity-input')) {
  customElements.define('ta-quantity-input', QuantityInputElement)
}
...
//
// Not derived from `InstanceType<typeof QuantityInputElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `QuantityInputElement`'s type, not a lazy type alias), that conditional type
...
export type QuantityInputProps = ComponentProps<typeof Component>
