...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/input/Rating.vue'
...
export const RatingElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-rating')) {
  customElements.define('ta-rating', RatingElement)
}
...
//
// Not derived from `InstanceType<typeof RatingElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `RatingElement`'s type, not a lazy type alias), that conditional type
...
export type RatingProps = ComponentProps<typeof Component>
