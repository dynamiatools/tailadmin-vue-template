...
import Component from '@dynamia-tools/tailadmin-vue/components/ui/Modal.vue'
...
export const ModalElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-modal')) {
  customElements.define('ta-modal', ModalElement)
}
...
//
// Not derived from `InstanceType<typeof ModalElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ModalElement`'s type, not a lazy type alias), that conditional type
...
export type ModalProps = ComponentProps<typeof Component>
