...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/scheduling/TimeSlotPicker.vue'
...
export const TimeSlotPickerElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-time-slot-picker')) {
  customElements.define('ta-time-slot-picker', TimeSlotPickerElement)
}
...
//
// Not derived from `InstanceType<typeof TimeSlotPickerElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `TimeSlotPickerElement`'s type, not a lazy type alias), that conditional type
...
export type TimeSlotPickerProps = ComponentProps<typeof Component>
