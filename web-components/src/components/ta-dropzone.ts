...
import Component from '@dynamia-tools/tailadmin-vue/components/forms/FormElements/Dropzone.vue'
...
export const DropzoneElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-dropzone')) {
  customElements.define('ta-dropzone', DropzoneElement)
}
...
//
// Not derived from `InstanceType<typeof DropzoneElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `DropzoneElement`'s type, not a lazy type alias), that conditional type
...
export type DropzoneProps = ComponentProps<typeof Component>
