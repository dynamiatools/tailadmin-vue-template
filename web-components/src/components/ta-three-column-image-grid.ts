...
import Component from '@dynamia-tools/tailadmin-vue/components/ui/images/ThreeColumnImageGrid.vue'
...
export const ThreeColumnImageGridElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-three-column-image-grid')) {
  customElements.define('ta-three-column-image-grid', ThreeColumnImageGridElement)
}
...
//
// Not derived from `InstanceType<typeof ThreeColumnImageGridElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ThreeColumnImageGridElement`'s type, not a lazy type alias), that conditional type
...
export type ThreeColumnImageGridProps = ComponentProps<typeof Component>
