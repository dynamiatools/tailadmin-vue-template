...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/Landing.vue'
...
export const LandingElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-landing')) {
  customElements.define('ta-landing', LandingElement)
}
...
//
// Not derived from `InstanceType<typeof LandingElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `LandingElement`'s type, not a lazy type alias), that conditional type
...
export type LandingProps = ComponentProps<typeof Component>
