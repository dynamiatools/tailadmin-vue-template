...
import Component from '@dynamia-tools/tailadmin-vue/components/profile/DangerZone.vue'
...
export const DangerZoneElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-danger-zone')) {
  customElements.define('ta-danger-zone', DangerZoneElement)
}
...
//
// Not derived from `InstanceType<typeof DangerZoneElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `DangerZoneElement`'s type, not a lazy type alias), that conditional type
...
export type DangerZoneProps = ComponentProps<typeof Component>
