...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/Hero.vue'
...
export const HeroElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-hero')) {
  customElements.define('ta-hero', HeroElement)
}
...
//
// Not derived from `InstanceType<typeof HeroElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `HeroElement`'s type, not a lazy type alias), that conditional type
...
export type HeroProps = ComponentProps<typeof Component>
