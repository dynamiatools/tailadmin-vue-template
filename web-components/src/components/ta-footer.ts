...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/Footer.vue'
...
export const FooterElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-footer')) {
  customElements.define('ta-footer', FooterElement)
}
...
//
// Not derived from `InstanceType<typeof FooterElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `FooterElement`'s type, not a lazy type alias), that conditional type
...
export type FooterProps = ComponentProps<typeof Component>
