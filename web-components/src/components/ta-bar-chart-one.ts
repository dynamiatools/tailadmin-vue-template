...
import Component from '@dynamia-tools/tailadmin-vue/components/charts/BarChart/BarChartOne.vue'
...
export const BarChartOneElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-bar-chart-one')) {
  customElements.define('ta-bar-chart-one', BarChartOneElement)
}
...
//
// Not derived from `InstanceType<typeof BarChartOneElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `BarChartOneElement`'s type, not a lazy type alias), that conditional type
...
export type BarChartOneProps = ComponentProps<typeof Component>
