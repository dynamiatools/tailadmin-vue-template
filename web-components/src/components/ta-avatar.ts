...
import Component from '@dynamia-tools/tailadmin-vue/components/ui/Avatar.vue'
...
export const AvatarElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-avatar')) {
  customElements.define('ta-avatar', AvatarElement)
}
...
//
// Not derived from `InstanceType<typeof AvatarElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `AvatarElement`'s type, not a lazy type alias), that conditional type
...
export type AvatarProps = ComponentProps<typeof Component>
