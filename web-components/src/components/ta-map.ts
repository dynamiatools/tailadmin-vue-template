...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/display/Map.vue'
...
export const MapElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-map')) {
  customElements.define('ta-map', MapElement)
}
...
//
// Not derived from `InstanceType<typeof MapElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `MapElement`'s type, not a lazy type alias), that conditional type
...
export type MapProps = ComponentProps<typeof Component>
