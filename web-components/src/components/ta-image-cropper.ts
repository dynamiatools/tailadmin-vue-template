...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/media/ImageCropper.vue'
...
export const ImageCropperElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-image-cropper')) {
  customElements.define('ta-image-cropper', ImageCropperElement)
}
...
//
// Not derived from `InstanceType<typeof ImageCropperElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ImageCropperElement`'s type, not a lazy type alias), that conditional type
...
export type ImageCropperProps = ComponentProps<typeof Component>
