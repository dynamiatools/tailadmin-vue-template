...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/commerce/Cart.vue'
...
export const CartElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-cart')) {
  customElements.define('ta-cart', CartElement)
}
...
//
// Not derived from `InstanceType<typeof CartElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `CartElement`'s type, not a lazy type alias), that conditional type
...
export type CartProps = ComponentProps<typeof Component>
