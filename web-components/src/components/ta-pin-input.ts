...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/input/PinInput.vue'
...
export const PinInputElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-pin-input')) {
  customElements.define('ta-pin-input', PinInputElement)
}
...
//
// Not derived from `InstanceType<typeof PinInputElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `PinInputElement`'s type, not a lazy type alias), that conditional type
...
export type PinInputProps = ComponentProps<typeof Component>
