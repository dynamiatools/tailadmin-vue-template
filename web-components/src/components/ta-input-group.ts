...
import Component from '@dynamia-tools/tailadmin-vue/components/forms/FormElements/InputGroup.vue'
...
export const InputGroupElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-input-group')) {
  customElements.define('ta-input-group', InputGroupElement)
}
...
//
// Not derived from `InstanceType<typeof InputGroupElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `InputGroupElement`'s type, not a lazy type alias), that conditional type
...
export type InputGroupProps = ComponentProps<typeof Component>
