...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/input/ScannerInput.vue'
...
export const ScannerInputElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-scanner-input')) {
  customElements.define('ta-scanner-input', ScannerInputElement)
}
...
//
// Not derived from `InstanceType<typeof ScannerInputElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ScannerInputElement`'s type, not a lazy type alias), that conditional type
...
export type ScannerInputProps = ComponentProps<typeof Component>
