...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/navigation/Kanban.vue'
...
export const KanbanElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-kanban')) {
  customElements.define('ta-kanban', KanbanElement)
}
...
//
// Not derived from `InstanceType<typeof KanbanElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `KanbanElement`'s type, not a lazy type alias), that conditional type
...
export type KanbanProps = ComponentProps<typeof Component>
