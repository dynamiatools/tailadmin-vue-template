...
import Component from '@dynamia-tools/tailadmin-vue/components/profile/AddressCard.vue'
...
export const AddressCardElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-address-card')) {
  customElements.define('ta-address-card', AddressCardElement)
}
...
//
// Not derived from `InstanceType<typeof AddressCardElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `AddressCardElement`'s type, not a lazy type alias), that conditional type
...
export type AddressCardProps = ComponentProps<typeof Component>
