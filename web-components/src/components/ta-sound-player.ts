...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/media/SoundPlayer.vue'
...
export const SoundPlayerElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-sound-player')) {
  customElements.define('ta-sound-player', SoundPlayerElement)
}
...
//
// Not derived from `InstanceType<typeof SoundPlayerElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `SoundPlayerElement`'s type, not a lazy type alias), that conditional type
...
export type SoundPlayerProps = ComponentProps<typeof Component>
