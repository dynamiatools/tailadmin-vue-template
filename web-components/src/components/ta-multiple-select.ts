...
import Component from '@dynamia-tools/tailadmin-vue/components/forms/FormElements/MultipleSelect.vue'
...
export const MultipleSelectElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-multiple-select')) {
  customElements.define('ta-multiple-select', MultipleSelectElement)
}
...
//
// Not derived from `InstanceType<typeof MultipleSelectElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `MultipleSelectElement`'s type, not a lazy type alias), that conditional type
...
export type MultipleSelectProps = ComponentProps<typeof Component>
