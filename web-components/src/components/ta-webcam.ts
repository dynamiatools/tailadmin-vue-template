...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/media/Webcam.vue'
...
export const WebcamElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-webcam')) {
  customElements.define('ta-webcam', WebcamElement)
}
...
//
// Not derived from `InstanceType<typeof WebcamElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `WebcamElement`'s type, not a lazy type alias), that conditional type
...
export type WebcamProps = ComponentProps<typeof Component>
