...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/input/MoneyInput.vue'
...
export const MoneyInputElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-money-input')) {
  customElements.define('ta-money-input', MoneyInputElement)
}
...
//
// Not derived from `InstanceType<typeof MoneyInputElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `MoneyInputElement`'s type, not a lazy type alias), that conditional type
...
export type MoneyInputProps = ComponentProps<typeof Component>
