...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/utilities/LazyLoader.vue'
...
export const LazyLoaderElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-lazy-loader')) {
  customElements.define('ta-lazy-loader', LazyLoaderElement)
}
...
//
// Not derived from `InstanceType<typeof LazyLoaderElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `LazyLoaderElement`'s type, not a lazy type alias), that conditional type
...
export type LazyLoaderProps = ComponentProps<typeof Component>
