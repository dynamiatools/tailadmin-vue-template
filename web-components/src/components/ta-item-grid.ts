...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/commerce/ItemGrid.vue'
...
export const ItemGridElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-item-grid')) {
  customElements.define('ta-item-grid', ItemGridElement)
}
...
//
// Not derived from `InstanceType<typeof ItemGridElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ItemGridElement`'s type, not a lazy type alias), that conditional type
...
export type ItemGridProps = ComponentProps<typeof Component>
