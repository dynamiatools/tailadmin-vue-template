...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/media/SignaturePad.vue'
...
export const SignaturePadElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-signature-pad')) {
  customElements.define('ta-signature-pad', SignaturePadElement)
}
...
//
// Not derived from `InstanceType<typeof SignaturePadElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `SignaturePadElement`'s type, not a lazy type alias), that conditional type
...
export type SignaturePadProps = ComponentProps<typeof Component>
