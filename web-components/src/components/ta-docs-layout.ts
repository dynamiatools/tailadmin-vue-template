...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/layouts/DocsLayout.vue'
...
export const DocsLayoutElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-docs-layout')) {
  customElements.define('ta-docs-layout', DocsLayoutElement)
}
...
//
// Not derived from `InstanceType<typeof DocsLayoutElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `DocsLayoutElement`'s type, not a lazy type alias), that conditional type
...
export type DocsLayoutProps = ComponentProps<typeof Component>
