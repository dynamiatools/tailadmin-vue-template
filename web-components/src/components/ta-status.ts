...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/display/Status.vue'
...
export const StatusElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-status')) {
  customElements.define('ta-status', StatusElement)
}
...
//
// Not derived from `InstanceType<typeof StatusElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `StatusElement`'s type, not a lazy type alias), that conditional type
...
export type StatusProps = ComponentProps<typeof Component>
