...
import Component from '@dynamia-tools/tailadmin-vue/components/tables/TableRow.vue'
...
export const TableRowElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-table-row')) {
  customElements.define('ta-table-row', TableRowElement)
}
...
//
// Not derived from `InstanceType<typeof TableRowElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `TableRowElement`'s type, not a lazy type alias), that conditional type
...
export type TableRowProps = ComponentProps<typeof Component>
