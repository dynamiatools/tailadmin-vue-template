...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/navigation/Menu.vue'
...
export const MenuElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-menu')) {
  customElements.define('ta-menu', MenuElement)
}
...
//
// Not derived from `InstanceType<typeof MenuElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `MenuElement`'s type, not a lazy type alias), that conditional type
...
export type MenuProps = ComponentProps<typeof Component>
