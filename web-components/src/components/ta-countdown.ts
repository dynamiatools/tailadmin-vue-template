...
import Component from '@dynamia-tools/tailadmin-vue/components/common/CountDown.vue'
...
export const CountdownElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-countdown')) {
  customElements.define('ta-countdown', CountdownElement)
}
...
//
// Not derived from `InstanceType<typeof CountdownElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `CountdownElement`'s type, not a lazy type alias), that conditional type
...
export type CountdownProps = ComponentProps<typeof Component>
