...
import Component from '@dynamia-tools/tailadmin-vue/components/profile/Security.vue'
...
export const SecurityElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-security')) {
  customElements.define('ta-security', SecurityElement)
}
...
//
// Not derived from `InstanceType<typeof SecurityElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `SecurityElement`'s type, not a lazy type alias), that conditional type
...
export type SecurityProps = ComponentProps<typeof Component>
