...
import Component from '@dynamia-tools/tailadmin-vue/components/profile/Modal.vue'
...
export const ProfileModalElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-profile-modal')) {
  customElements.define('ta-profile-modal', ProfileModalElement)
}
...
//
// Not derived from `InstanceType<typeof ProfileModalElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ProfileModalElement`'s type, not a lazy type alias), that conditional type
...
export type ProfileModalProps = ComponentProps<typeof Component>
