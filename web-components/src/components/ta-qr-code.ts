...
import Component from '@dynamia-tools/tailadmin-vue/components/ext/display/QrCode.vue'
...
export const QrCodeElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-qr-code')) {
  customElements.define('ta-qr-code', QrCodeElement)
}
...
//
// Not derived from `InstanceType<typeof QrCodeElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `QrCodeElement`'s type, not a lazy type alias), that conditional type
...
export type QrCodeProps = ComponentProps<typeof Component>
