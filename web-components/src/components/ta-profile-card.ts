...
import Component from '@dynamia-tools/tailadmin-vue/components/profile/ProfileCard.vue'
...
export const ProfileCardElement = defineCustomElement(Component, { shadowRoot: false })

if (typeof customElements !== 'undefined' && !customElements.get('ta-profile-card')) {
  customElements.define('ta-profile-card', ProfileCardElement)
}
...
//
// Not derived from `InstanceType<typeof ProfileCardElement>` even though defineCustomElement's own
// typing looks like it should give this for free: across this package's build (declaration emit
// resolving the *value* `ProfileCardElement`'s type, not a lazy type alias), that conditional type
...
export type ProfileCardProps = ComponentProps<typeof Component>
