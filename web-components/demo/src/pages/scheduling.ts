...
// README's "Known limitations" section.
  {
    const el = mount('ta-date-range-picker', document.getElementById('demo-ta-date-range-picker')!, { modelValue: null })
    on(el, 'update:modelValue', (v) => { el.modelValue = v })
  }
  {
    const el = mount('ta-time-slot-picker', document.getElementById('demo-ta-time-slot-picker')!, {
    slots: [
