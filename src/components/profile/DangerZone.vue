<template>
  <div class="mb-6 rounded-2xl border border-gray-200 p-5 lg:p-6 dark:border-gray-800">
    <h4 class="text-lg font-semibold text-gray-800 mb-3 lg:mb-6 dark:text-white/90">{{ title }}</h4>
    <div>
      <div
      ...
      >
        <div>
          <span class="block text-base mb-1 font-medium text-gray-800 dark:text-white/90">{{
            row1Title
          }}</span>
...
// Row copy defaults match upstream verbatim, including the fact that both rows'
// title/description read "Logout all devices" / "Sign out from every active session"
// even though the buttons are "Change Password" and "Delete account" — that
// mismatch is an upstream content bug, not something fixed silently here. Pass
// your own row1Title/row2Title etc. to correct it.
const {
  title = 'Danger Zone',
  row1Title = 'Logout all devices',
  row1Description = 'Sign out from every active session.',
  row1ButtonLabel = 'Change Password',
  row2Title = 'Logout all devices',
  row2Description = 'Sign out from every active session.',
  row2ButtonLabel = 'Delete account',
} = defineProps<{
  title?: string
  row1Title?: string
  row1Description?: string
  row1ButtonLabel?: string
  row2Title?: string
  row2Description?: string
  row2ButtonLabel?: string
}>()
